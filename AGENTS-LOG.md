...
### 2026-09-08 (AI layer) — Claude Code — branch: claude/pen-ai-layer
- **Claiming:** `lib/pen/*`, `app/pen/*`, `app/api/pen/*`. Nothing else.
- Why: a second user appeared (a PA at a hospital in FL). The realtor-only extraction — reactions-per-room, objections, buying signals — is meaningless for a clinical meeting, so the extraction had to generalise before any UI work mattered.
- Did:
  - **`lib/pen/extract.ts` rewritten as auto-detected templates.** ONE Claude call now classifies the meeting (`showing` / `clinical` / `generic`) AND extracts, so a non-technical user configures nothing. Universal fields: headline, summary, **people** (name/role/speaker/note), decisions, **actions** (owner/due/priority), open_questions, and **missed** — the "you might have missed" block, which is the most valuable section and the hardest for a generic notetaker to copy. `showing` extras only fill for viewings. Meeting type is overridable from the UI, which re-runs extraction.
  - **PRIVACY BY DESIGN in the prompt, and it is load-bearing.** The model is instructed to record the ACTION and drop the sensitive detail behind it ("chase the referral for Tuesday's admission", not the diagnosis) and never to copy identifiers. These notes live outside the hospital's systems, so derived PHI is kept to the minimum that keeps the note useful.
  - **`lib/pen/chat.ts` + `app/api/pen/chat/route.ts` — grounded Q&A over one transcript.** Answers from the recording only; says "that is not in the recording" rather than reasoning from general knowledge. Transcript sits in `system` behind a `cache_control` breakpoint, so follow-up questions on the same recording read it from cache instead of paying for it again. History capped at 40 turns per session.
  - **UI rewritten as a document, not a dashboard** (`app/pen/pen-theme.css` + `PenApp.tsx`). Instrument Serif titles / Literata prose / IBM Plex Mono meta, warm paper ground, one slate-blue accent with amber reserved for warnings, 68ch measure. People as chips with initials, actions as persisted checkboxes, "you might have missed" gets its own hatched texture. Chat is a sticky right drawer with type-aware starter questions. Staggered reveal on the note sections, `prefers-reduced-motion` respected. His own notes still come FIRST — what he wrote is the spine, the extraction is supporting material.
  - Consent copy widened to cover the clinical case: "…and this recording contains no patient or medical information."
- **DB: one ALTER needed, and it is at the bottom of `lib/pen/schema.sql` as a single line** (multi-line DDL kept failing in the Supabase editor — the editor splits it). Adds `meeting_type text`, `chat jsonb default '[]'`, `action_done jsonb default '[]'`. Until it runs, the note renders but chat and action ticks fail with a clear in-panel error.
- Checks: `npx tsc --noEmit` ✅ · `npm run build` ✅ (8 pen routes incl. the new `/api/pen/chat`). Runtime-verified against a seeded session: full note renders, type override present, chat panel + type-aware chips render, and the missing-column error surfaces cleanly in the panel rather than silently failing.
- ⚠ **A seed row `__SEED_PREVIEW__.wav` was inserted into `pen_sessions`** to verify the design without real audio. Delete it when done. There is also a REAL earlier session in there titled "This transcript is not a real estate showing" — that is the old realtor-only prompt failing on non-showing audio, i.e. the exact problem this branch fixes.
- **HIPAA, unresolved and it gates the clinical use case.** All three vendors sign BAAs but: Supabase needs the **Team plan** (~$599/mo), and **Anthropic is enterprise-sales only with no list pricing**. Private storage is NOT HIPAA compliance. Real patient meetings must not be uploaded until those are signed.
- Next: still the same blocking test — one real recording, read the diarized transcript. Also unbuilt: pushing actions out to somewhere she'd actually see them (email/calendar), and a Gemini-vs-AssemblyAI comparison he asked about (Gemini diarization is prompt-based and its sync request model fights the 60s function ceiling — test before switching).

### 2026-09-09 — Claude Code — branch: claude/pen-agentic
- **Claiming:** `lib/pen/*`, `app/pen/*`, `app/api/pen/*`. **New dependency: `motion@13.2.0`** — other agents must `npm install`.
- Built all four features Agustin specced for an "agentic workspace", each verified against his real recordings (his Mercor interview, 63 diarized utterances) rather than fixtures.
- **1 — Jot and Enhance.** Provenance is per BLOCK, not per character: each paragraph is its own borderless textarea, so editing is ordinary controlled state and there is no contenteditable (no selection/paste/IME class of bug). Black = you wrote it, grey = machine, and editing a grey block promotes it to black because correcting a line is an act of ownership. **Enhance appends, never rewrites** — rewriting in place would make the distinction meaningless and lose what was actually typed. Returns an empty expansion when the transcript does not support one. Transcript is editable as an OVERLAY keyed by utterance index so AssemblyAI's original is never destroyed.
- **2 — Cmd+K archive chat.** Two-stage orchestration: a compact index (title/date/type/summary/people) selects which recordings matter, then only those are loaded and the answer must cite them. One-prompt-with-everything works at four recordings and dies at four hundred. Model emits `[n]` markers + a parallel citation list; unmatched markers are dropped rather than left as dead text. History in `pen_archive_chat`.
- **3 — Instant deliverables.** Draft email / memo / tracker from a SINGLE item (action, missed thing, open question), revealed on hover, always visible on touch. Drafts leave marked gaps like `[candidate name]` rather than inventing specifics — confirmed in practice. Emails carry To:/Subject: inside the body so one Copy takes everything. iOS-style bottom sheet, editable before copying.
- **4 — Briefing email.** Compiles summary / missed / actions / still-open into an email-safe HTML template (inline styles, tables, no webfonts) and sends via the existing Resend helper. **Recipient is always the signed-in user** — no client-supplied address, or it becomes an open relay. Added `GET /api/pen/briefing?id=` to preview the exact HTML without sending.
- **Bugs found and fixed while building (all real, all caught by testing rather than review):**
  - The save indicator said "saved" on a failed PATCH. `onPatch` now rethrows and the editor shows **"not saved"** in red. An editor that lies about saving is worse than one with no indicator.
  - `motion` writes its own `transform` for enter animations, which silently clobbered the palette's CSS `translateX(-50%)` and pushed the sheet off-centre. Centring is now flex on a wrapper.
  - `parsed_output` only exists on `messages.parse()`; I had used `messages.create()` with a cast. Switched.
  - Copy relied solely on `navigator.clipboard`, which needs a secure context AND permission and is absent in several in-app browsers. Added the `execCommand` fallback.
  - The email leaked `&RARR;` and `&MIDDOT;` — `esc()` escapes the ampersand in an HTML entity. Labels now use literal Unicode, and a charset meta was added. **Never pass entities through `esc()`.**
  - `briefing_sent_at` was PATCHed by the UI but missing from the route's allowlist, so it silently 400'd. Added with timestamp validation.
- Schema: ONE line adds `note_blocks`, `transcript_edits`, `deliverables`, `briefing_sent_at`; one more creates `pen_archive_chat`. Both at the bottom of `lib/pen/schema.sql`. Already run on production Supabase. (Supabase's SQL editor splits multi-line DDL — that is why everything is one line.)
- Checks: `npx tsc --noEmit` ✅ · `npm run build` ✅ (11 pen routes). A real briefing email was sent to his address during testing.
- ⚠ **HIPAA unchanged and it now matters more.** The briefing email is the one path that pushes note content to a third party (Resend) and into a mail server. Extraction already strips identifiers and keeps clinical detail out — that is exactly why. Still no BAAs: Supabase needs Team (~$599/mo), Anthropic is enterprise-sales only.
- Known, not fixed: import dedupe keys on the COMPRESSED byte count, and the mono/16kHz conversion is not bit-identical between runs, so the same file imports twice (the archive palette noticed this on its own). Should key on original size. There is also still no way to delete a recording from the UI.
