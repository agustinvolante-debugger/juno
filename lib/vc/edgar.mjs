...
// entity classification: operating_company vs vc_firm, with an honest confidence flag.
// Signals: the Form D industry taxonomy, and fund-name patterns in the issuer name.
const FUND_STRONG = /\b(fund|funds|ventures?|capital|partners|angels?|SPV|syndicate)\b|\bL\.?P\.?$|,?\s*(a )?series of/i;
const FUND_WEAK = /\b(invest(ments?|ing)?|holdings?|equity|opportunit(y|ies)|growth|seed|GP)\b/i;
const FIN_INDUSTRY = /^$|pooled|other$|invest|banking|financial|insurance|REITS|real estate/i;
export function classifyIssuer(name, industryGroup) {
  const n = name || '', ig = industryGroup || '';
  if (/pooled investment fund/i.test(ig)) return { type: 'vc_firm', conf: 'high' };
  const strong = FUND_STRONG.test(n), weak = FUND_WEAK.test(n);
  const finIg = FIN_INDUSTRY.test(ig);
  if (strong) return { type: 'vc_firm', conf: finIg ? 'high' : 'low' };
  if (weak) return { type: finIg ? 'vc_firm' : 'operating_company', conf: 'low' };
  return { type: 'operating_company', conf: ig ? 'high' : 'low' };
}
...
export async function upsertUniverse(sb, cik, doc, filingDate) {
  ...
  const cls = classifyIssuer(isNewer ? doc.issuerName : ex?.name, isNewer ? (doc.industryGroup || ex?.industry_group) : ex?.industry_group);
  const row = {
    cik: id,
    name: isNewer ? doc.issuerName : ex.name,
    norm_name: normName(isNewer ? doc.issuerName : ex.name),
    entity_type: cls.type,
    type_confidence: cls.conf,
